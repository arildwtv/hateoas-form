...
function setProperties(config, resource, properties) {
  ...
  var updateResourcePromise = (0, _updateResource2.default)(config, resource, nonLinkProperties);
  ...
  var allPromises = Object.keys(linkProperties).reduce(function (acc, linkPropertyName) {
    ...
    return acc.then(function (updatedResource) {
      return Promise.all([updatedResource, (0, _getEmbeddedResource2.default)(config, updatedResource, linkPropertyName)]);
    }).then(function (_ref) {
      ...
      if (!embeddedResource) {
        ...
        return Promise.all([updatedResource, (0, _createResourceAndSetProperties2.default)(config, linkHref, propertiesForEmbeddedResource, setProperties)]);
      }
      ...
      var embeddedResourcePromise = Array.isArray(embeddedResource) ? Promise.all((0, _mergeArrays2.default)(new Array(embeddedResource.length), propertiesForEmbeddedResource).map(function (propertiesForItem, index) {
        if (!embeddedResource[index]) {
          ...
          return (0, _createResourceAndSetProperties2.default)(config, _linkHref, propertiesForItem, setProperties);
        }
